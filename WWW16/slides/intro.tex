...
\frame{
  ...
  Definition:
  \begin{itemize}
    \item closeness centrality $C(x)$ of a  user $x$
      $$
      C(i)=1/\sum_{y\neq x \in V}d(y,x).
      $$
      %\item \footnotesize
    \item $d(y,x)$ is the length of a shortest path between nodes $y$ and $x$.
    ...
    \begin{column}{55mm}
      ...
      \begin{itemize}
        ...
        \item[\footnotesize $\sigma_{st}$]:  number of different shortest paths
          between nodes $s$ \& $t$
        \item[\footnotesize $\sigma_{st}(x)$:] how many of them pass through node $x$
      \end{itemize}
    \end{column}
    \begin{column}{45mm}
